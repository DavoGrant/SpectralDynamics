...
# Fitting config.
concat_table = 'Hkappa_RegimeConcat2'
jd_tuples = [{'label': 'constant-1', 'template': 'Harvard_GMOS_C1',
              'solver': 'CF', 'jd_rule': (2455036.526, 'before')},

             {'label': 'constant-1', 'template': 'Harvard_GMOS_C2',
              'solver': 'CF', 'jd_rule': (2455036.526, 'after')},
...
for regime in jd_tuples:
    ...
    fitter.add(etacar_templates.h_i_3750(version=regime['template'],
                                         solver=regime['solver']))

    # Ready fitting routine and pre-processing options.
    fitter.rcs_edge_size = (5, 16)
