...
# Fitting config.
concat_table = 'Hgamma_RegimeConcat8'
jd_tuples = [{'label': 'in-apastron', 'template': 'Harvard_GMOS_A1',
              'solver': 'CF', 'jd_rule': (2454779.839, 'before')},

             {'label': 'in-apastron-nw', 'template': 'Harvard_GMOS_A2',
              'solver': 'CF', 'jd_rule': ((2454779.839, 2454840.731), 'between')},

             {'label': 'early-periastron', 'template': 'Harvard_GMOS_P1',
              'solver': 'CF', 'jd_rule': ((2454840.731, 2454852.828), 'between')},

             {'label': 'mid-periastron', 'template': 'Harvard_GMOS_P2',
              'solver': 'CF', 'jd_rule': ((2454852.828, 2454907.778), 'between')},

             {'label': 'out-apastron-nw', 'template': 'Harvard_GMOS_P3',
...
for regime in jd_tuples:
    ...
    fitter.add(etacar_templates.h_i_4340(version=regime['template'],
